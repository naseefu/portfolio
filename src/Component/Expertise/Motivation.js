...
const Motivation = () => {
  ...
  return (
    ...
    <div className='grid grid-cols-1 md:grid-cols-2 justify-between w-[90%] mx-auto md:mt-[120px] md:pt-10 pb-5 mb-10 text-left gap-10'>
      <div className='expert2 md:min-h-[50vh]'>
        <h1 ref={(el)=>sectionsRef.current[0]=el} className='expert2card font-bold md:sticky md:top-20 text-[35px] md:text-[5vw] xl:text-[60px]'>MOTIVATION</h1>
      </div>
      <div className='expert2 font-[600] flex flex-col gap-10 text-[20px] text-gray-300'>
        <p ref={(el)=>sectionsRef.current[1]=el} className='expert2card'>More than a role, software development is a journey of innovation and creativity for me. It’s an opportunity to transform abstract ideas into 
           tangible solutions that solve problems and enhance lives. Whether it’s crafting seamless user interfaces, 
           developing robust backend systems, or integrating cutting-edge technologies, each project allows me to push boundaries and explore new possibilities.</p>
        <p ref={(el)=>sectionsRef.current[2]=el} className='expert2card'>As a developer, I thrive on the blend of logic and creativity, where every line of code contributes to a larger vision. The dynamic nature of the tech world keeps me inspired – one day, I’m brainstorming ideas for a task management app; the next, 
          I’m implementing complex APIs or designing scalable databases. Every challenge sharpens my skills, and the reward of seeing my work in action motivates me to keep innovating.</p>
      </div>
    </div>
 ...
 API integration, and building dynamic front-end interfaces with ReactJS.</p>
          <p>I contributed to end-to-end development, from conceptualization to deployment.</p>
        </div>
      </div>
      </div>
      ...
      <div className='lg:min-h-[50vh] w-[fit-cotent]'>
        ...
        <h1 ref={(el)=>sectionsRef.current[7]=el} className='expert2card'>FAVOURITE</h1>
        <h1 ref={(el)=>sectionsRef.current[8]=el} className='expert2card -mt-3 md:-mt-6'>STACK</h1>
        </div>
      </div>
      <div className='expert2 font-[600] flex flex-col gap-10 text-[20px] text-gray-300 cols-span-1.3 lg:w-[70%] max-w-[850px]'>
        {stack.map((s,index)=>(
          <div ref={(el)=>sectionsRef.current[9+index]=el} className='expert2card flex flex-col md:flex-row gap-4 md:gap-6 items-start px-4 py-6 md:px-8 md:py-8 bg-[#0d0d0d] rounded-xl ' style={{border:"1px solid rgba(255,255,255,0.06)"}}>
            <div className='w-[fit-content] px-3 rounded-xl py-3 bg-[#0d0d0d]' style={{border:'1px solid rgba(255,255,255,0.06)'}}>
